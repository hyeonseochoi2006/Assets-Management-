...
PRODUCT DATA AUDIT / SOURCE IDENTITY
...
- SOURCE_MISMATCHED means the evidence belongs to another instrument or could not safely support this product. Do not quote that rejected number as product data and do not describe it as an official-data conflict.
- SOURCE_UNVERIFIED means the source-to-product identity was not established; treat the number as UNVERIFIED.
- If the Source Identity Guardrail reports SOURCE_CONTAMINATION_REJECTED, explain concisely that unrelated/mismatched source data was discarded.
- If AUM, shares outstanding, NAV, expense ratio, benchmark, leverage target, reset frequency, derivatives structure, or prospectus warning is marked CONFLICT or UNVERIFIED, do not present it as a confirmed fact.
- If Product Data Auditor was unavailable, clearly state that the product figures were not independently verified.

CORRELATION REPORTING
- CORRELATION_STATUS is internal metadata and must never be printed literally.
- If CORRELATION_STATUS: UNVERIFIED, do not state that the candidate has high/low/positive/negative measured correlation with existing holdings.
- You may describe verified sector, industry, benchmark, or holdings overlap separately, but do not call overlap "correlation".
- Do not claim diversification improves or worsens because of measured correlation unless CORRELATION_STATUS: VERIFIED.
...
- Never invent prices, portfolio values, holdings, target weights, maximum position sizes, sector caps, risk limits, risk scores, correlation, or market data.
...
4. 데이터 검증: STOCK은 Data Auditor, ETF 계열은 Product Data Auditor의 핵심 VERIFIED/CONFLICT/UNVERIFIED와 source contamination만 요약
5. 포트폴리오 적합성: 상관관계는 VERIFIED인 경우에만 수치/방향 단정
...
def run_korean_ceo_brief(
    ...
    context_parts.append(
        "Prepare the Korean CEO report now. Preserve the instrument type, audit status, source-identity status, facts, and numeric values exactly. "
        ...
        "Respect CORRELATION_STATUS; do not invent measured correlation. "
        "Do not invent any investor limit, risk score, numeric wipeout threshold, correlation claim, or new recommendation. Preserve the CIO's materiality and escalation judgment."
    )
    ...
    report = "\n".join(
        ...
        and not line.strip().startswith("CORRELATION_STATUS:")
    )
